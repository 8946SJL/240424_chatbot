...
# OpenAI API 키 설정 (API 키 입력 필요)
# openai.api_key = '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'
...
#     # 전체 데이터 열에서 쿼리 검색
#     matching_rows = df_jobs[df_jobs.astype(str).apply(lambda x: x.str.contains(query, na=False)).any(axis=1)]

#     if not matching_rows.empty:
#         # 완벽히 일치하는 직업을 DataFrame에 추가
#         for idx, job in matching_rows.iterrows():
#             job_with_similarity = job.to_frame().T
#             job_with_similarity['similarity'] = 1.0
#             similar_jobs = pd.concat([similar_jobs, job_with_similarity])

#     # 유사도 기준으로 최종 정렬
#     similar_jobs.sort_values(by='similarity', ascending=False, inplace=True)

#     # 최종 결과를 상위 top_k로 제한
#     similar_jobs = similar_jobs.head(top_k)

#     if similar_jobs.empty:
#         st.warning("유사한 직업을 찾을 수 없습니다.")
#         return pd.DataFrame()
...
#     else:
#         system_content = "당신은 노인과 대화를 나눌 수 있는 친근한 AI입니다. 주 사용 목적은 일자리 정보 제공 입니다. "
#         user_content = f"사용자의 질문: {user_input}\n\n이 질문에 대해 대화를 나눠 주세요."
...
#     # DB에서 final_jobs.csv 데이터를 불러오기
#     query = "SELECT * FROM final_jobs"  # 적절한 쿼리로 final_jobs.csv 데이터를 가져옴
#     df_jobs = pd.read_sql(query, conn)  # SQL 쿼리를 사용하여 데이터 가져오기
...
#                         context = similar_jobs[['title', 'category', 'address', 'pay', 'detail']].to_string(index=False)
